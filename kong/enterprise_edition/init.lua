	...
	init = {
		after = function ()
			...
			hooks.register_hook("api:init:pre", function (app)
				...
				app:match("/:workspace_name/kong", "/:workspace_name/kong", app_helpers.respond_to(slash_handler))
			...
			local function prepend_workspace_prefix(app, route_path, methods)
				if route_path ~= "/" then
					app:match("workspace_" .. route_path, "/:workspace_name" .. route_path, app_helpers.respond_to(methods))
				end
			...
			hooks.register_hook("balancer:get_peer:pre", function (target_host)
				return tracing.trace("balancer.getPeer", {
					qname = target_host
				})
			end)
			...
			hooks.register_hook("balancer:to_ip:pre", function (target_host)
				return tracing.trace("balancer.toip", {
					qname = target_host
				})
			end)
...
function _M:prepare_portal(kong_config)
	...
	local portal_developer_meta_fields = workspace_config.retrieve(ws_constants.PORTAL_DEVELOPER_META_FIELDS, workspace, opts) or "[]"
...
function _M.license_hooks(config)
	...
	hooks.register_hook("fips:kong:validate", function (l_type)
		if l_type == "free" then
			kong.log.warn("FIPS mode is not supported in Free mode. Please reach out to " .. "Kong if you are interested in using Kong FIPS compliant artifacts")
		...
		kong.log.warn("enabling FIPS mode on ", openssl_version.version_text, " (", openssl_version.version(openssl_version.CFLAGS), ")")
	end)
