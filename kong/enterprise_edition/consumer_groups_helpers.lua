...
local function _select_consumer_group(consumer_group_pk)
	...
	return kong.db.consumer_groups:select({
		id = consumer_group_pk
	})
end
...
local function _find_consumer_group_config(consumer_group_pk, plugin_name)
	for row, err in kong.db.consumer_group_plugins:each_for_consumer_group({
		id = consumer_group_pk
	}, nil, {
		search_fields = {
			name = {
...
local function get_consumers_in_group(consumer_group_pk)
	...
	for row, err in kong.db.consumer_group_consumers:each_for_consumer_group({
		id = consumer_group_pk
	}) do
		len = len + 1
...
local function select_by_username_or_id(db, key)
	return not utils.is_valid_uuid(key) and db:select_by_username(key) or db:select({
		id = key
	})
end
...
local function get_groups_by_consumer(consumer_pk)
	...
	for row, err in kong.db.consumer_group_consumers:each_for_consumer({
		id = consumer_pk
	}) do
		if err then
			return nil, err
