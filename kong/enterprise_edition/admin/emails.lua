...
function _M:register_url(email, jwt, username)
	return fmt("%s/register?email=%s&username=%s", self.admin_gui_url, ngx.escape_uri(email), ngx.escape_uri(username)) .. (jwt and fmt("&token=%s", ngx.escape_uri(jwt) or "") or "")
end
...
function _M:invite(recipients, jwt)
	...
	for _, recipient in ipairs(recipients) do
		if not recipient.email or not recipient.username then
			return nil, {
				code = 500,
				message = "recipient does not have username or email"
			}
		end

		options.html = fmt(template.html, self:register_url(recipient.email, jwt, recipient.username), recipient.username, kong_conf.admin_invitation_expiry / 60 / 60, ADMIN_DOCS_URL)
...
function _M:reset_password(email, jwt)
	...
	local reset_url = (self.kong_conf.admin_gui_url or "") .. "/account/reset-password?email=" .. ngx.escape_uri(email) .. "&token=" .. ngx.escape_uri(jwt)
	local options = {
		from = self.kong_conf.admin_emails_from,
		reply_to = self.kong_conf.admin_emails_reply_to,
		subject = fmt(template.subject),
		html = fmt(template.html, reset_url, reset_url, self.kong_conf.admin_invitation_expiry / 60 / 60)
	}
	local res = self.client:send({
		email
	}, options)
