...
function _M.new(opts)
	...
	if opts.client_max_frame_size ~= nil and (type(opts.client_max_frame_size) ~= "number" or opts.client_max_frame_size < 1) then
		error("opts.client_max_frame_size must be a number >= 1", 2)
	end

	if opts.client_max_fragments ~= nil and (type(opts.client_max_fragments) ~= "number" or opts.client_max_fragments < 1) then
		error("opts.client_max_fragments must be a number >= 1", 2)
	end

	if opts.upstream_max_frame_size ~= nil and (type(opts.upstream_max_frame_size) ~= "number" or opts.upstream_max_frame_size < 1) then
		error("opts.upstream_max_frame_size must be a number >= 1", 2)
	end

	if opts.upstream_max_fragments ~= nil and (type(opts.upstream_max_fragments) ~= "number" or opts.upstream_max_fragments < 1) then
	...
	if opts.lingering_time and opts.lingering_timeout and opts.lingering_time < opts.lingering_timeout then
...
local function send_close(self, target, source, status, reason)
	...
	self:dd("sending close frame to ", target, ", initiator: ", source, ", status: ", status, ", reason: '", reason, "'")
...
local function forwarder_close(self, role, code, data, peer_code, peer_data)
	local client_status, client_reason, upstream_status, upstream_reason = nil
	...
	return close(self, "proxy", client_status, client_reason, upstream_status, upstream_reason)
end
...
local function forwarder(proxy, self, peer)
	...
	local co = co_running()
	local ws = self.ws
	local peer_ws = peer.ws

	while true do
		if peer.state == STATE.CLOSING then
			log(DEBUG, "peer (", peer.role, ") has been sent a close frame, ", role, " forwarder exiting")

			return role
		elseif peer.state == STATE.CLOSED then
			log(DEBUG, "peer (", peer.role, ") connection is closed, ", role, " forwarder exiting")
		...
		elseif self.state == STATE.CLOSING then
			...
			if lingering_time < now() - self.close_sent then
				log(INFO, "lingering time expired waiting for ", role, " to send more data, forwarder exiting")
		...
		if not data then
			...
			elseif find(err, "exceeding max payload len", 1, true) then
				log(INFO, role, " recv() frame size exceeds limit ", "(", max_frame_size, "), closing")
		...
		if STATE.CLOSED <= peer.state then
			return role
		end
		...
		if typ then
			if not opcode then
				log(ERR, "NYI - ", role, " sent unknown frame type: ", typ, " (dropping connection)")
			...
			if proxy.debug and (not err or typ == "close") then
				...
				if DEBUG_PAYLOAD_MAX_LEN < #payload then
				...
				proxy:dd(fmt("\n[frame] downstream %s resty.proxy %s upstream\n" .. "  aggregating: %s\n" .. "  type: \"%s\"%s\n" .. "  payload: %s (len: %d)\n" .. "  fin: %s", arrow, arrow, proxy.aggregate_fragments, typ, extra, fmt("%q", payload), data and #data or 0, fin))
			end

			local bytes = nil
			local forward = true
			local data_frame = typ == "text" or typ == "binary" or typ == "continuation"

			if data_frame then
				frame_size = frame_size + #data

				if max_frame_size and max_frame_size < frame_size then
					log(INFO, role, " frame size (", frame_size, ") exceeds", "  limit (", max_frame_size, "), closing")
					forwarder_close(proxy, role, 1009, "Payload Too Large", 1001, "")
				...
				if max_fragments and max_fragments < frame_count then
					log(INFO, role, " frame count (", frame_count, ") ", "exceeds limit (", max_fragments, "), closing")
					forwarder_close(proxy, role, 1009, "Payload Too Large", 1001, "")
			...
			if forward then
				if on_frame then
					local updated, updated_code = on_frame(proxy, role, typ, data, fin, code)

					if updated ~= nil and type(updated) ~= "string" then
						error("opts.on_frame return value must be " .. "nil or a string")
					end
					...
					if typ == "close" and updated_code ~= nil then
						if type(updated_code) ~= "number" then
							error("opts.on_frame status code return value " .. "must be nil or a number")
						end
				...
				if on_frame and data == nil then
					...
					if typ == "close" then
						...
						if not bytes then
							log(ERR, "failed forwarding frame from ", role, ": ", err)
						end
					end
				end
...
function _M:connect_upstream(uri, opts)
	if self.upstream then
		log(WARN, "connection with upstream (", self.upstream_uri, ")", " already established")
