...
local function get_expiry_and_cache_ttl(token, ttl)
	...
	if type(token) == "table" then
		...
		if not expires_in and token.exp then
	...
	elseif expires_in == 0 then
		...
		if ttl.max_ttl and ttl.max_ttl > 0 and ttl.max_ttl < expires_in then
			cache_ttl = ttl.max_ttl
...
local function parse_jwt_response(oic, body, headers, ignore_signature, hint)
	...
	if type(headers) == "table" then
		...
		if type(content_type) == "string" and (find(content_type, "application/jwt", 1, true) == 1 or find(content_type, hint, 1, true) == 1) then
			local decoded, err = oic.token:decode(body, ignore_signature and TOKEN_DECODE_OPTS or TOKEN_DECODE_SIGNATURE_OPTS)
...
local function init_worker()
	...
	if kong.configuration.database == "off" then
		...
		kong.worker_events.register(function (issuer)
			...
			if data and discovery_data and discovery_data.n > 0 then
				for i = discovery_data.n, 1, -1 do
	...
	kong.worker_events.register(function (data)
		...
		local old_username, old_custom_id = nil
...
local function fetch_configuration(issuer, opts)
	...
	if rediscovery_lifetime > seconds_since_last_rediscovery then
		log.notice("rediscovery for ", issuer, " was done recently (", rediscovery_lifetime - seconds_since_last_rediscovery, " seconds until next rediscovery)")
...
local function fetch_keys(jwks_uri, opts)
	...
	if rediscovery_lifetime > seconds_since_last_rediscovery then
		log.notice("rediscovery for ", jwks_uri, " was done recently (", rediscovery_lifetime - seconds_since_last_rediscovery, " seconds until next rediscovery)")
...
local function decode_previous_configuration(issuer_entity, issuer)
	...
	if type(configuration_decoded) ~= "table" then
		log.err("decoding previous discovery document failed: ", err or "unknown error", " (falling back to empty configuration)")
...
local function decode_previous_jwks(jwks_uri, jwks_string, err)
	if jwks_cache[jwks_uri] then
		log.notice("loading jwks from ", jwks_uri, " thread failed: ", jwks_string or "unknown error", " (falling back to previous jwks)")

		local jwks_uri_jwks = nil
	...
	log.notice("loading jwks from ", jwks_uri, " failed: ", err or "unknown error", " (ignoring)")
end
...
local function discover(issuer, opts, issuer_entity)
	opts = opts or {}
	local configuration_decoded, err = nil
	...
	configuration_decoded = configuration_decoded or config_fallback(issuer_entity, issuer)
	...
	if jwks_uris_count > 0 then
		...
		for i = 1, jwks_uris_count do
			...
			elseif type(jwks_string) ~= "string" then
				jwks_uri_jwks = decode_previous_jwks(jwks_uri, jwks_string, err)
			else
				...
				elseif jwks_cache[jwks_uri] then
	...
	if jwk_count > 0 then
		...
		if type(jwks) ~= "string" then
			if issuer_entity then
				log.notice("encoding jwks keys failed: ", err or "unknown error", " (falling back to previous keys)")

				jwks = issuer_entity.keys
			else
				log.err("encoding jwks keys failed: ", err or "unknown error", " (falling back to empty keys)")

				jwks = "[]"
			end
		end
	elseif issuer_entity then
	...
	if type(encoded) ~= "string" then
		if issuer_entity then
			log.notice("encoding discovery document failed: ", err or "unknown error", " (falling back to previous configuration)")

			encoded = issuer_entity.configuration
		else
			log.err("encoding discovery document failed: ", err or "unknown error", " (falling back to empty configuration)")
...
local function rediscover(issuer, identifier, opts, issuer_entity)
	issuer_entity = issuer_entity or issuer_select(identifier)
	...
	if issuer_entity then
		...
		if kong.configuration.database == "off" then
			...
			local stored_data, err = kong.db.oic_issuers:upsert({
				id = issuer_entity.id
			}, data)

			if not stored_data then
				log.warn("unable to upsert issuer ", data.issuer, " discovery documents in database (", err or "unknown error", ")")
			else
				data = stored_data
			end
		end
...
local issuers = {
	rediscover = function (issuer, opts)
		...
		if issuer_entity then
			...
			if type(configuration_decoded) == "table" then
				...
				log.notice("rediscovery failed when decoding discovery for ", identifier, " (", err, ")")
			end
			...
			if rediscovery_lifetime > seconds_since_last_rediscovery then
				log.notice("rediscovery for ", identifier, " was done recently (", rediscovery_lifetime - seconds_since_last_rediscovery, " seconds until next rediscovery)")
		...
		if not rediscovery_semaphore then
			...
			if err then
				log.warn("rediscovery was unable to create a semaphore for ", identifier, " (", err, ")")
			else
				rediscovery_semaphores[identifier] = rediscovery_semaphore
			end
		end

		local locked, new_issuer_entity = nil
		...
		if rediscovery_semaphore then
			for i = 1, 5 do
				...
				if new_issuer_entity then
					...
					local new_updated_at = type(configuration_decoded) == "table" and configuration_decoded.updated_at or 0
...
local function log_multiple_matches(subject, matches)
	...
	log.notice(fmt("multiple consumers match '%s' by username case-insensitively: %s", subject, concat(match_info, ", ")))
end
...
local function consumers_load(subject, key, by_username_ignore_case)
	...
	if key == "id" then
		if utils.is_valid_uuid(subject) then
			result, err = kong.db.consumers:select({
				id = subject
			})
		end
...
function consumers.load(subject, anonymous, consumer_by, ttl, by_username_ignore_case)
	...
	for _, field_name in ipairs(field_names) do
		if field_name ~= "id" or utils.is_valid_uuid(subject) then
			local key = nil
...
function kong_oauth2.load(ctx, access_token, ttl, use_cache)
	...
	if exp > 0 then
		...
		if token_ttl > 0 then
			if ttl.max_ttl and ttl.max_ttl > 0 and ttl.max_ttl < token_ttl then
				token_ttl = ttl.max_ttl
			end

			if ttl.min_ttl and ttl.min_ttl > 0 and token_ttl < ttl.min_ttl then
...
local function get_token_cache_key(iss, salt, args)
	...
	elseif args.grant_type == "client_credentials" then
		assert(args.client_id and args.client_secret or args.assertion, "no credentials given for client credentials grant")
	...
	if args.token_cache_key_include_scope and scope and args.grant_type ~= "refresh_token" then
...
local function token_exchange_load(endpoint, opts)
	...
	if httpc.set_proxy_options and (opts.http_proxy or opts.https_proxy) then
