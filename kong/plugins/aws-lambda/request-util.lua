...
local function extract_proxy_response(content)
	local serialized_content, err = nil
...
local function aws_serializer(ctx, config)
	...
	for qname, qvalue in pairs(queryStringParameters) do
		if type(qvalue) == "table" then
			...
			multiValueQueryStringParameters[qname] = {
				qvalue
			}
		end
	end

	local body, isBase64Encoded = nil
...
local function build_request_payload(conf)
	...
	elseif conf.forward_request_body or conf.forward_request_headers or conf.forward_request_method or conf.forward_request_uri then
	...
	if not upstream_body_json then
		kong.log.err("could not JSON encode upstream body", " to forward request values: ", err)
	end
